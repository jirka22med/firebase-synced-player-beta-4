...
let currentPlaylist = [];
// Ukládáme si, zda byl původní HTML audio element ztlumen, předtím než se ujme Web Audio API
let wasAudioPlayerMuted = false;
...
    console.log("loadAudioData: Spouštím načítání dat pro audio přehrávač.");
...
            if (loadedFirestorePlayerSettings.currentTrackIndex !== undefined) currentTrackIndex = loadedFirestorePlayerSettings.currentTrackIndex;
            // Nastavení hlasitosti: Použijeme rovnou hodnotu z Firestore, Web Audio API ji pak převezme
            // Max hodnota slideru je nyní 3 (pro 300%)
            if (loadedFirestorePlayerSettings.volume !== undefined && volumeSlider) volumeSlider.value = loadedFirestorePlayerSettings.volume;
            // Uložení stavu ztlumení pro Web Audio API
            if (loadedFirestorePlayerSettings.muted !== undefined) wasAudioPlayerMuted = loadedFirestorePlayerSettings.muted;
...
        if (savedPlayerSettings.volume !== undefined && volumeSlider) volumeSlider.value = savedPlayerSettings.volume;
        if (savedPlayerSettings.muted !== undefined) wasAudioPlayerMuted = savedPlayerSettings.muted; // Použijeme pro Web Audio API
...
    console.log("saveAudioData: Spouštím ukládání všech dat audio přehrávače do LocalStorage a Firebase.");
...
        // Uložíme aktuální hodnotu ze slideru, která je 0-3
...
    // Změněno z `confirm` na vlastní `showNotification` pro lepší UI
    window.showNotification("Opravdu chcete smazat VŠECHNA data audio přehrávače? Tato akce nelze vrátit zpět! Potvrďte v konzoli.", 'warn', 10000);
    console.warn("Potvrďte smazání dat v konzoli vývojářských nástrojů (pokud je otevřena).");

    // Pro účely Canvasu použijeme zjednodušenou logiku nebo instrukci pro uživatele
    // jelikož confirm() není doporučeno v iframu.
    // V reálné aplikaci by zde byl vlastní modal s tlačítky OK/Zrušit.
    if (confirm('JSTE SI ABSOLUTNĚ JISTI? Všechna data audio přehrávače budou nenávratně ztracena!')) {
        ...
        console.log("clearAllAudioPlayerData: Mazání všech dat audio přehrávače zrušeno uživatelem.");
    }
};
...
// Funkce logarithmicVolume již není potřeba, protože Web Audio API GainNode pracuje s lineárními hodnotami gainu
// v rozsahu 0-3 pro 0-300%.


function updateVolumeDisplayAndIcon() {
    if (!audioPlayer || !volumeSlider || !muteButton || !volumeValueElement) return;
    // Získáváme hodnotu přímo ze slideru, která je 0-3
    const sliderValue = parseFloat(volumeSlider.value);
    // Vypočítáme procenta pro zobrazení (sliderValue * 100)
    const displayPercentage = Math.round(sliderValue * 100);

    // Mute stav kontrolujeme primárně přes audioPlayer.muted (který nastavujeme v mute/unmute funkci)
    // a sekundárně, pokud je slider velmi blízko nule.
    if (audioPlayer.muted || sliderValue <= 0.005) { // Upraveno pro sliderValue blížící se nule
        muteButton.textContent = '🔇';
        volumeValueElement.textContent = '0';
    } else {
        volumeValueElement.textContent = displayPercentage;
        // Logika ikon hlasitosti upravena pro rozsah slideru 0-3 (0-300%)
        if (sliderValue <= 0.33) muteButton.textContent = '🔈'; // Do cca 33%
        else if (sliderValue <= 1.0) muteButton.textContent = '🔉'; // Do 100%
        else if (sliderValue <= 2.0) muteButton.textContent = '🔊'; // Do 200%
        else muteButton.textContent = '💥'; // Nad 200% (až do 300%) - Nová ikona!
    }
}
...
    audioPlayer.load(); // Nyní to načte HTML audio element, ale hlasitost řídí Web Audio API
    // Vytvoříme novou Promise, která se vyřeší, jakmile je zvuk připraven k přehrávání
    const playPromise = new Promise((resolve, reject) => {
        audioPlayer.addEventListener('canplaythrough', () => resolve(), { once: true });
        audioPlayer.addEventListener('error', (e) => reject(e), { once: true });
    });

    playPromise.then(() => {
        trackTitleElement.textContent = track.title;
        // Spustíme přehrávání HTML audio elementu, i když hlasitost je 0 (řídí ji GainNode)
        audioPlayer.play().then(async () => {
            console.log("playTrack: Přehrávám:", track.title);
        ...
        console.error('playTrack: Chyba při načítání mediálních dat:', error);
        window.showNotification(`Chyba při načítání dat pro: ${track.title}. Možná špatná URL.`, 'error');
        updateButtonActiveStates(false);
    });
}
...
// ***********************************************************************************************************
// TADY JE HLAVNÍ MÍSTO, KTERÉ ŘÍDÍ HLASITOST S BOOSTEREM
// ***********************************************************************************************************
if (volumeSlider && audioPlayer) volumeSlider.addEventListener('input', async (e) => {
    // console.log("Volume slider changed.");
    if (window.updateWebAudioVolume) { // Kontrola, zda je Web Audio API booster načten a připraven
        window.updateWebAudioVolume(parseFloat(e.target.value)); // Předáme hodnotu slideru (0-3) boosteru
    } else {
        // Fallback pro případ, že volume-booster.js ještě není načten nebo aktivní
        // V tomto případě je hlasitost omezena na 100%, protože GainNode není aktivní
        // Mapujeme hodnotu slideru (0-3) na rozsah 0-1 pro HTML audio element.
        audioPlayer.volume = Math.min(1, parseFloat(e.target.value) / 3);
    }
    updateVolumeDisplayAndIcon(); // Aktualizujeme zobrazení hlasitosti a ikonu
    await saveAudioData(); // Uložíme nový stav hlasitosti
});
// ***********************************************************************************************************
// KONEC HLAVNÍHO MÍSTA PRO HLASITOST
// ***********************************************************************************************************


if (muteButton && audioPlayer && volumeSlider) muteButton.addEventListener('click', async () => {
    console.log("Mute button clicked.");
    audioPlayer.muted = !audioPlayer.muted; // Stále udržujeme HTML audio.muted pro interní stav

    if (audioPlayer.muted) {
        muteButton.dataset.previousVolume = volumeSlider.value; // Uložíme aktuální hodnotu slideru
        volumeSlider.value = 0; // Nastavíme slider na 0 (pro zobrazení)
        // Pokud je Web Audio API aktivní, nastavíme mu gain na 0 pro ztlumení
        if (window.getWebAudioMainGainNode) {
            const mgNode = window.getWebAudioMainGainNode();
            if (mgNode) mgNode.gain.value = 0;
        }
    } else {
        // Pokud je odtlumeno, obnovíme původní hlasitost ze slideru
        const prevSliderVol = muteButton.dataset.previousVolume || '0.5'; // Výchozí 0.5 (50%)
        volumeSlider.value = prevSliderVol; // Nastavíme slider zpět na původní hodnotu
        // Pokud je Web Audio API aktivní, nastavíme mu hlasitost z previousVolume
        if (window.updateWebAudioVolume) {
            window.updateWebAudioVolume(parseFloat(prevSliderVol));
        } else {
            // Fallback pro HTML audio, mapujeme zpět na 0-1 rozsah
            audioPlayer.volume = Math.min(1, parseFloat(prevSliderVol) / 3);
        }
    }
    updateVolumeDisplayAndIcon(); // Aktualizujeme zobrazení a ikonu
    await saveAudioData(); // Uložíme nový stav ztlumení
});

if (audioPlayer) {
    // Odstranili jsme listener na 'volumechange' z audioPlayeru,
    // protože nyní hlasitost přímo ovládá GainNode a updateVolumeDisplayAndIcon
    // je volán z input eventu volumeSlideru a muteButtonu.
    // audioPlayer.addEventListener('volumechange', updateVolumeDisplayAndIcon); // Tato linka je duplicitní a odstraněna.
...
        console.log("Audio skončilo. Přehrávám další skladbu, pokud není opakování.");
        updateButtonActiveStates(false);
        if (!audioPlayer.loop) playNextTrack();
        await saveAudioData();
        console.log("Stav přehrávače uložen po skončení skladby.");
    });
...
        console.error("Chyba audio přehrávače:", e);
...
    console.log(`Stisknuta klávesa: ${e.code}`);
...
        case 'KeyA':
            if (volumeSlider) {
                // Zde upravíme rozsah pro krok hlasitosti s ohledem na 0-300%
                volumeSlider.value = Math.max(0, parseFloat(volumeSlider.value) - 0.05);
                volumeSlider.dispatchEvent(new Event('input')); // Simulujeme "input" událost pro aktualizaci
                await saveAudioData();
            }
            break;
        case 'KeyD':
            if (volumeSlider) {
                // Zde upravíme rozsah pro krok hlasitosti s ohledem na 0-300%
                volumeSlider.value = Math.min(3, parseFloat(volumeSlider.value) + 0.05);
                volumeSlider.dispatchEvent(new Event('input')); // Simulujeme "input" událost pro aktualizaci
                await saveAudioData();
            }
            break;
...
    }
    ...
    // Inicializace Web Audio API boostu
    // Musí být voláno až po načtení audioPlayer elementu a skriptu volume-booster.js
    if (audioPlayer && typeof window.initWebAudio === 'function') {
        window.initWebAudio(audioPlayer);
        // Po inicializaci Web Audio API, nastavíme hlasitost na základě uložené hodnoty
        if (volumeSlider) {
            window.updateWebAudioVolume(parseFloat(volumeSlider.value));
            // Aplikujeme ztlumení, pokud bylo uloženo při načítání dat
            if (wasAudioPlayerMuted) {
                const mgNode = window.getWebAudioMainGainNode();
                if (mgNode) mgNode.gain.value = 0;
            }
        }
    } else {
        console.warn("DOMContentLoaded: window.initWebAudio funkce nebo audioPlayer element nejsou k dispozici. Web Audio Booster nebude inicializován.");
        // Fallback pro HTML audio player, pokud Web Audio API není k dispozici
        if (audioPlayer && volumeSlider) {
            // Mapujeme slider value (0-3) zpět na HTML audio volume (0-1) pro fallback
            audioPlayer.volume = Math.min(1, parseFloat(volumeSlider.value) / 3);
            audioPlayer.muted = wasAudioPlayerMuted;
        }
    }
